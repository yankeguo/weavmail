...
def trash(mail_files: tuple[str, ...], sync_limit: int):
    """Move mails to the account's configured trash mailbox, then sync.

    MAIL_FILE is the local .md file path. Multiple files can be passed for batch
    operations.

    The trash mailbox is read from each mail's account trash_mailbox setting.
    An error is raised if trash_mailbox is not configured for any account.
    ...
    for f in mail_files:
        ...
        if not dst_mailbox:
            click.echo(
                f"Error: Account '{front_account}' does not have trash_mailbox configured. "
                f"Run `weavmail mailbox --account {front_account}` to list available folders, "
                f"then `weavmail account config {front_account} --trash-mailbox <MAILBOX>` to set it.",
                err=True,
            )
            raise SystemExit(1)
